...
def find_color1(frame):
    ...
    hsv_lowerbound = np.array([52,54,114]) #replace THIS LINE w/ your hsv lowerb
    hsv_upperbound = np.array([125,166,255])#replace THIS LINE w/ your hsv upperb
...
def find_color2(frame):
    ...
    """
    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv_lowerbound =  np.array([59,00,2])#replace THIS LINE w/ your hsv lowerb
    hsv_upperbound = np.array([110, 99, 105])#replace THIS LINE w/ your hsv upperb
    mask = cv2.inRange(hsv_frame, hsv_lowerbound, hsv_upperbound)
